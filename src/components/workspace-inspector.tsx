...
export default function WorkspaceInspector({ activeView, drawing, messages, components, reviewComponents, selected, onViewChange, onSelect, onConfirm, onEdit, onRemove, onRegenerate, onExport }: {
  ...
  const [{ permission }, notification] = XNotification.useNotification();
  const exportWithNotice = () => {
    if (permission === "granted") notification.open({ title: "正在准备元件分析清单", body: "Excel 生成完成后将自动下载。", tag: "drawing-export", duration: 3000 });
    onExport();
  };

  const items = [
    { key: "drawing", label: <span><AppstoreOutlined /> 图纸</span>, children: <DrawingInspector drawing={drawing} /> },
    { key: "files", label: <span><FileOutlined /> 文件</span>, children: <SessionFilesPanel drawing={drawing} messages={messages} onExport={exportWithNotice} /> },
    ...
    { key: "bom", label: <span><TableOutlined /> BOM</span>, children: <BomInspector drawing={drawing} onRegenerate={onRegenerate} onExport={exportWithNotice} /> },
  ];
